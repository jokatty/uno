const socket = io();
let match = false;
async function checkMatchedCard(matchedCardDiv) {
  // document.getElementById("td1").getElementsByTagName('div')[0].innerHTML
  // const matchedCardInnerHtml = matchedCardDiv.inner;
  const suit = matchedCardDiv.getElementsByTagName('h3')[0].innerText;
  console.log(suit);
  const rank = matchedCardDiv.getElementsByTagName('p')[0].innerText;
  console.log(rank);

  if (match == 'true') {
    console.log('hey, this is a match');
    // when match is found, make a axio request to remove the card from the pile. pop a new card
    // see if this works for change the card
  ...
  // UPDATE THE DATABASE WITH THE REMOVED CARD INFO
  // call delete
  // const playerName = req.cookies.userName;
  // consolr.log(`loggedIn user is: ${playerName}`);
  // console.log(`matched card: ${matchedCardInnerHtml}`);

  //  START FROM HERE. WE HAVE THE OBJECT, SEND THIS TO THE cards/delete route.
  const deleteObj = {
    suit,
    rank,
  };
  // await axios.delete('/cards', { deleteObj }).then((response) => {
  //   console.log(response);
  // }).catch((err) => {
  //   console.log(err);
  // });
  // app.delete('/cards', cardController.deleteCard);
  // DO THE DELETE LOGIC IN THE ROUTE

  const viewNext = document.createElement('button');
  viewNext.innerText = 'Next card';
  document.body.appendChild(viewNext);
